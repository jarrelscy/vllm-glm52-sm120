...
__global__ void pack_planes(const half* in, unsigned* out, unsigned char* sc,
  ...
  if (partners && i < slots) {
    ...
    int key = cold[i] >= 0  ? cold[i] + 1
              : hot[i] >= 0 ? (hot[i] + 1) | 0x40000000
                            : 0;
    unsigned matches = __match_any_sync(__activemask(), key);
    unsigned before = matches & ((1u << lane) - 1u);
    unsigned after = matches & ~(0xffffffffu >> (31 - lane));
    partners[i] = key == 0               ? -1
                  : (__popc(before) & 1) ? -2
                  : after                ? (i / 32) * 32 + __ffs(after) - 1
                                         : -1;
  }
  int k = i % K, slot = i / K;
...
__global__ void hybrid_kernel(const unsigned* cw, const unsigned* cb,
                              ...
                              int S, int P, int slots, const int* partners) {
  __shared__ unsigned lut[384];
  int slot = blockIdx.z;
  int partner = partners[slot];
  if (partner == -2) return;
  int cold_e = cold_ids[slot], hot_e = hot_ids[slot];
  bool cold = cold_e >= 0;
  if (cold) {
    for (int i = threadIdx.x; i < 384; i += blockDim.x) lut[i] = cb[i];
    __syncthreads();
  }
  int lane = threadIdx.x & 31, q = lane / 4, c = lane % 4,
      warp = threadIdx.x / 32, tile = blockIdx.x * 4 + warp;
  if (tile >= N / 16) return;
...
#pragma unroll
        for (int j = 0; j < 4; j++)
          a[j] = hw[(((((long long)hot_e * (N / 16) + tile) * G + g) * 4 + j) *
                     32) +
                    lane];
  ...
  if (partner < 0) {
    float a = ldexpf(d0, -8 * c) + ldexpf(d1, -8 * c - 4),
          b = ldexpf(d2, -8 * c) + ldexpf(d3, -8 * c - 4);
    ...
    float a = ldexpf(d0, -8 * (c % 2)) + ldexpf(d1, -8 * (c % 2) - 4),
          b = ldexpf(d2, -8 * (c % 2)) + ldexpf(d3, -8 * (c % 2) - 4);
    a += __shfl_xor_sync(0xffffffff, a, 1);
    b += __shfl_xor_sync(0xffffffff, b, 1);
    slot = c < 2 ? slot : partner;
    if ((c == 0 || c == 2) && slot >= 0 && slot < slots) {
      partial[((long long)slot * N + tile * 16 + q) * S + blockIdx.y] = a;
      partial[((long long)slot * N + tile * 16 + q + 8) * S + blockIdx.y] = b;
    }
  }
...
extern "C" int hybrid_launch(const void* cold_w, const void* cold_cb,
                             ...
                             void* partial, void* out, const void* partners,
                             float cold_global, int N, int K, int slots,
                             int split, int P, int hot_parts, void* stream) {
  if (N <= 0 || N % 16 || K <= 0 || K % 128 || slots <= 0 || split <= 0 ||
      (P != 4) || (hot_parts != 1 && hot_parts != 2))
  ...
  hybrid_kernel<<<dim3((N + 63) / 64, split, slots), 128, 0, s>>>(
      ...
      split, P, slots, (const int*)partners);
  cudaError_t err = cudaGetLastError();
  if (err != cudaSuccess) return (int)err;
  hybrid_reduce<<<((long long)slots * N + 255) / 256, 256, 0, s>>>(
      (const float*)partial, (float*)out, (const int*)cold_ids,
      (const int*)hot_ids, (const float*)hot_global, cold_global, N, slots,
      split, hot_parts);
  return (int)cudaGetLastError();
}

// Pair consecutive occurrences within each warp of routed slots. Invalid slots
// remain single so the math kernel explicitly writes their defined zero output.
__global__ void build_pairs(const int* cold, const int* hot, int* partner,
                            int slots) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i >= slots) return;
  int lane = threadIdx.x & 31;
  ...
  int key = cold[i] >= 0  ? cold[i] + 1
            : hot[i] >= 0 ? (hot[i] + 1) | 0x40000000
                          : 0;
  unsigned matches = __match_any_sync(active, key);
  if (key == 0) {
    partner[i] = -1;
    return;
  }
  unsigned before = matches & ((1u << lane) - 1u);
  int ordinal = __popc(before);
  if (ordinal & 1) {
    partner[i] = -2;
    return;
  }
  unsigned after = matches & ~(0xffffffffu >> (31 - lane));
  partner[i] = after ? ((i / 32) * 32 + __ffs(after) - 1) : -1;
}
extern "C" int routed_pairs(const void* cold, const void* hot, void* pairs,
                            int slots, void* stream) {
  if (slots <= 0) return (int)cudaErrorInvalidValue;
  build_pairs<<<(slots + 127) / 128, 128, 0, (cudaStream_t)stream>>>(
      (const int*)cold, (const int*)hot, (int*)pairs, slots);
  return (int)cudaGetLastError();
}
