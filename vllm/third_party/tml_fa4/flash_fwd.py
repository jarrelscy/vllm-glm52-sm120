...
from vllm.third_party.tml_fa4 import ampere_helpers as sm80_utils
...
class FlashAttentionForwardBase:
    ...
    # arch used to pick the acc_O -> smem store atom in epilogue(). None = use the
    # real detected device arch. Subclasses whose __call__ uses a different MMA
    # family than the device's native one set this (see FlashAttentionForwardSm80).
    _store_atom_arch_override: Optional[int] = None
    ...
    def epilogue(
        ...
        smem_copy_atom_O = utils.get_smem_store_atom(
            self._store_atom_arch_override
            if self._store_atom_arch_override is not None
            else self.arch.major * 10 + self.arch.minor,
            self.dtype,
        )
...
class FlashAttentionForwardSm80(FlashAttentionForwardBase):
    ...
    # Ampere m16n8k16 MMA, so the acc_O -> smem store atom must be the arch<90
    # CopyUniversalOp, not StMatrix (SM90+ GMMA layout). self.arch is the REAL
    # device arch, so on sm_120 (major=12 >= 9) the default would wrongly pick
    # StMatrix and scramble odd 8-wide head_dim N-subtiles -- same self.arch
    # pitfall already handled for use_tma_O. Force the Ampere store atom.
    _store_atom_arch_override: Optional[int] = 80
