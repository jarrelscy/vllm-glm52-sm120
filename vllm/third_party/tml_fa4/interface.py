...
def _flash_attn_fwd(
    ...
    # SM120 relative bias is applied via the score_mod set above (raw rel_logits
    # as aux_tensors[0]); the sheared/TMA `bias` tensor is not used. Keep
    # rel_extent set for compile_key correctness (per-rel_extent kernels differ).
    if rel_bias is not None and arch // 10 == 12:
        rel_extent = rel_bias.shape[-1]
        rel_extent_padded = 0
        bias = None
