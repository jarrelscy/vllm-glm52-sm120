...
def create_rel_bias_scoremod(rel_extent):
    ...
    @cute.jit
    def rel_bias_fn(acc_S_SSA, batch_idx, head_idx, q_idx, kv_idx, seqlen_info, aux_tensors):
        rel = aux_tensors[0]
        n = const_expr(cute.size(acc_S_SSA.shape))
        s = cute.make_fragment(acc_S_SSA.shape, Float32)
        s.store(acc_S_SSA)
        qv = cute.make_fragment(q_idx.shape, Int32)
        qv.store(q_idx)
        kvv = cute.make_fragment(kv_idx.shape, Int32)
        kvv.store(kv_idx)
        hv = cute.make_fragment(head_idx.shape, Int32)
        hv.store(head_idx)
