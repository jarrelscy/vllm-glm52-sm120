...
try:
    ...
    # nvidia-cutlass-dsl 4.6.x removes cutlass.cute.core.ThrMma, which
    # quack's package __init__ chain (rmsnorm -> copy_utils -> layout_utils)
    # references, so ANY quack import raises there. We only ever needed this
    # 15-line helper (verbatim from quack/compile_utils.py, Apache-2.0,
    # (c) 2025 Wentao Guo, Ted Zadouri, Tri Dao) — vendor it so the kernel
    # works on both cutlass-dsl 4.5.x (quack path) and 4.6.x (this path).
...
class StableTopKFromGatheredCandidatesKernel:
            ...
            for jj in cutlass.range_constexpr(kk):
                ...
                if cutlass.const_expr(j >= self.tb_size):
                    ...
                    for e in cutlass.range_constexpr(elems):
                        i = tid + Int32(e * self.tb_size)
                        a = vals[e]
                        b = Int32(cute.arch.shuffle_sync_bfly(a, offset=j))
                        mn = a
                        mx = b
                        if a > b:
                            mn = b
    ...
    def kernel(
        ...
        if cutlass.const_expr(self.packed_bytes > 0):
            input_row = input[None, row, None]
            input_words = cute.recast_tensor(input, Uint32)
        elif cutlass.const_expr(self.raw_world_size > 0):
        ...
        for key_idx in cutlass.range_constexpr(self.keys_per_thread):
            if cutlass.const_expr(self.raw_world_size > 0):
                ...
                if cutlass.const_expr(self.packed_bytes > 0):
                    score = recast_val(input_words[rank_c, row, col_local], Float32)
                    low = Uint32(input_row[rank_c, 8192 + col_local])
                    high = Uint32(input_row[rank_c, 10240 + col_local])
                    token_bits = low | (high << Uint32(8))
                    if cutlass.const_expr(self.packed_bytes == 7):
                        upper = Uint32(input_row[rank_c, 12288 + col_local])
                        token_bits = token_bits | (upper << Uint32(16))
                        token_id = Int32(token_bits)
                        if token_bits == Uint32(0xFFFFFF):
                            token_id = Int32(-1)
                    else:
                        token_id = Int32(token_bits) * Int32(4) + Int32(rank_c)
                        if token_bits == Uint32(0xFFFF):
                            token_id = Int32(-1)
                else:
    ...
    def compile(
        ...
        if packed_bytes:
            assert raw_world_size == 4 and num_candidates == 8192
            assert packed_bytes in (6, 7)
            row_bytes = packed_bytes * 2048
            gathered = cute.runtime.make_fake_tensor(
                Uint8,
                (4, num_rows, row_bytes),
                stride=(cute.sym_int64(divisibility=row_bytes), row_bytes, 1),
                assumed_align=4,
            )
        elif raw_world_size:
            per_rank = num_candidates // raw_world_size
        ...
        kernel = StableTopKFromGatheredCandidatesKernel(
            topk, num_candidates, canonical, raw_world_size, packed_bytes
        )
...
def select_packed(gathered, out):
    assert gathered.dtype == torch.uint8 and gathered.is_contiguous()
    assert gathered.ndim == 3 and gathered.shape[0] == 4
    packed_bytes = gathered.shape[2] // 2048
    assert gathered.shape[2] in (6 * 2048, 7 * 2048)
    assert out.shape == (gathered.shape[1], 2048) and out.dtype == torch.int32
    StableTopKFromGatheredCandidatesKernel.compile(2048, 8192, True, 4, packed_bytes)(
        gathered, out
    )
    return out
