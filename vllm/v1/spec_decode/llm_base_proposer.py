...
class SpecDecodeBaseProposer:
    ...
    def model_returns_tuple(self) -> bool:
        if self.method == "mtp":
            ...
            # recycle_hidden). Inkling's MTP returns a single raw tensor
            # (chain_hidden_post_norm=False: the same value feeds the muP
            # LM head and the next depth). Other MTP families return a
            # single tensor.
            archs = self.draft_model_config.hf_config.architectures or []
            return "DeepSeekMTPModel" in archs
