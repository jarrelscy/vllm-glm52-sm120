...
class AutoRegressiveSpeculator(DraftModelSpeculator):
    def __init__(self, vllm_config: VllmConfig, device: torch.device):
        ...
        # HACK: the Inkling MTP draft has no MM processor of its own (its
        # draft config carries the target's MM wrapper but the draft
        # architecture is text-only), yet it consumes the target's merged
        ...
        # Multi-module MTP (Inkling: 8 depth layers): the first
        # num_spec_prefill_steps draft tokens are produced by re-running the
        # draft prefill window once per depth module (hidden chained
        # module-to-module, ids shifted left one per step). The trailing
        # (num_spec_prefill_steps - 1) draft ids / input hidden states are
        # cached per request so the next round can re-prefill positions
        # invalidated by rejection.
    ...
    def capture(self) -> None:
        ...
        # When num_spec_prefill_steps > 1 (multi-module MTP), the speculator
        # builds its attention metadata using its own builders and buffers
        # (re-prefill shifts positions/seq_lens away from the target's).
        # Otherwise, draft prefill reuses the target model's attention
        # metadata at runtime, so capture builds its dummy metadata through
        # the target model runner's builders and buffers.
    ...
    def propose(
        ...
        if not skip_attn and self.num_spec_prefill_steps > 1:
            ...
            attn_metadata = self._build_draft_attn_metadata_from_qsl(
                num_reqs=num_reqs,
        ...
        if self.num_speculative_steps <= self.num_spec_prefill_steps:
            # Early exit: every draft token came from the prefill step loop.
        ...
        try:
            # Generate the remaining draft tokens (steps past the prefill
            # step loop).
    ...
    def _prefill(
        ...
        for step in range(self.num_spec_prefill_steps):
            ...
            # Run depth module `step` over the whole draft window. The
            # previous step's full-window output is the hidden input
            # (self.hidden_states was updated below), and input_ids were
            # shifted left by one with the previous draft token appended.
    ...
    def _multi_step_decode(
        ...
        first_step = self.num_spec_prefill_steps
        for step in range(first_step, self.num_speculative_steps):
            # Rebuild every step when positions advance, or just once
            # on the first step when positions are constant (Gemma4 MTP).
            if not skip_attn and (self.advance_draft_positions or step == first_step):
...
def _prepare_prefill_inputs_kernel(
    ...
    # num_reprefills to keep positions aligned with the attention's
